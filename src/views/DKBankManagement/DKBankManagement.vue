<template>
  <div  v-loading="loading">
    ...
    <el-table border stripe :data="myData" class="hongte-table" highlight-current-row>
      ...
      <el-table-column label="代扣类型" min-width="150" prop="type"></el-table-column>
      <el-table-column label="代扣渠道" min-width="150" prop="channel"></el-table-column>
      <el-table-column label="支持银行" min-width="150" prop="bankName"></el-table-column>
      <el-table-column label="单日限额(元)" min-width="150" prop="dailyLimit">
        <template slot-scope="scope">
          <p v-if="scope.row.dailyLimit=='0'">-</p>
          <p v-else>{{scope.row.dailyLimit}}</p>
        </template>
      </el-table-column>
       <el-table-column label="单笔限额(元)" min-width="150" prop="singleLimit">
        <template slot-scope="scope">
          <p v-if="scope.row.singleLimit=='0'">-</p>
          <p v-else>{{scope.row.singleLimit}}</p>
        </template>
      </el-table-column>
    ...
    <el-dialog class="hongte-dialog" width="80%" ref="dialog" :visible.sync='dialog.visible' :title='dialog.title'>
      <DialogModify :postData="dialog.postData" @closeDialog="closeDialog" @getData="getData"></DialogModify>
    </el-dialog>
  </div>
</template>

<script>
import DialogModify from './Dialog/DialogModify';
export default {
  name: 'DKBankManagement',
  data() {
    return {
      loading: false,
      // dialog
  ...
  methods: {
    getData() {
      ...
      this.$api.withholdingLimitGetList(params).then(res => {
        this.loading = false;
    ...
    //新增编辑
    addEdit() {
      this.dialog.title = '新增代扣限额信息';
      this.dialog.visible = true;
      this.dialog.postData = {};
    },
    //修改编辑
    handleEdit(row) {
      this.dialog.title = '修改代扣限额信息';
      this.dialog.visible = true;
