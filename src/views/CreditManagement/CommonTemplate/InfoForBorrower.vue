<template>
  <el-collapse-item class="collapse-head" title="借款人信息" name="loader">
    <el-button type="primary" size="small" @click="queryCustomerTogetherInfo()" v-if="where.productSeriesCode!='ML003'">
      <i class="el-icon-plus"></i> 新增共借人</el-button>
  
    <el-table border stripe :data="where.issueBusinessReqVos" ref="addLoader" class="hongte-table" highlight-current-row @row-click="handlerClickRow" @selection-change="handleSelectionChange">
      <el-table-column fixed type="selection" align="center" label="序号" width="50" :selectable="selectable"></el-table-column>
      ...
      <el-table-column label="可发标额度" v-if="where.productSeriesCode!='ML003'" prop="canBorrowLimit" min-width="100"></el-table-column>
  ...
  methods: {
    ...
    getData() {
      ...
      issueBusinessReqVos.forEach((x, index) => {
        ...
        //默认选中主借款人
        if (index == 0) {
          this.$refs.addLoader.toggleRowSelection(x, true);
          this.$set(x, 'pushIssueState', '1');
        }
      });
    },
    //查询借款额度查询
    queryBorrowLimit(row) {
      //渤海信托不需要发标额度----修改于2019-03-22
      if(this.where.productSeriesCode=='ML003'){
        return false
      }
      let borrowType = row.customerType == 'PERSON' ? '1' : '2';
      let identityCard = row.cardNumber || row.certificateNumber;
      this.$api
        .queryBorrowLimit({
    ...
    addLoader() {
      ...
      checkedAddLoader.forEach(x => {
        ...
        //pushIssueState: 推标状态(0.未上标  1. 已上标,默认为已上标)
        if (isNew) {
          x.pushIssueState = '1';
          addLoader.push(x);
        }
