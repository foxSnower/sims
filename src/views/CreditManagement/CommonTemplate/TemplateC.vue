...
<script>
import InfoForBorrower from './InfoForBorrower';
import InfoForContractAndOutMoneyForTemplateC from '@views/LetterManagement/CommonTemplate/InfoForContractAndOutMoneyForTemplateC';
...
export default {
  ...
  methods: {
    getData() {
      ...
      //       this.$set(this.where, 'creditResultId', res.data.creditResultId);
    ...
    saveAndSubmit() {
      ...
      this.$api.paymentEditorCommit(params).then(res => {
        this.submitLoading = false;
        if (res.returnCode == '0000') {
          this.result = true;
