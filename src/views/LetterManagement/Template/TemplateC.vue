  ...
  <el-card>
    <template v-if="!result">
      <el-collapse v-model="activeNames">
        ...
        <!-- 借款人信息 -->
        <InfoForBorrower v-model="where" :where="where" :total.sync="total" :filterProductInfo="filterProductInfo" ref="InfoForBorrower" type="house_loan"></InfoForBorrower>
        <!-- 出款信息/合同信息/担保主体信息 -->
        <InfoForContractAndOutMoneyForTemplateC :rules="rules" ref="InfoForContractAndOutMoney" v-model="where" :where="where" :isSub="isSub"></InfoForContractAndOutMoneyForTemplateC>
...
import InfoForContractAndOutMoneyForTemplateC from '../CommonTemplate/InfoForContractAndOutMoneyForTemplateC';
...
export default {
  ...
  data() {
    return {
      ...
      where: {
        ...
        relationName: '', //担保人主体联系人
        relationAddress: '', //担保地址
        relationTelephone: '', //担保联系电话
        relationEmail: '', //担保邮箱
  ...
  components: {
    InfoForBorrower,
    InfoForContractAndOutMoneyForTemplateC,
    ApplicationResult,
    HtSelect,
    HtInput
  },
  ...
  methods: {
        ...
        this.$api
          ...
          .then(res => {
            if (res.returnCode == '0000') {
              ...
              this.$set(this.where, 'guaranteeMain', res.data.guaranteeMain);
              ...
              this.$set(this.where, 'companyName', res.data.companyName); //所属分公司全称
              this.$set(this.where, 'companyAddress', res.data.companyAddress); //分公司地址
              this.$set(this.where, 'legalName', res.data.legalName); //法人代表
              this.$set(this.where, 'legalTelephone', res.data.legalTelephone); //联系电话
              this.$set(this.where, 'uniteCode', res.data.uniteCode); //统一社会信用代码
              this.$set(this.where, 'horizontalText', res.data.horizontalText); //电子签章横向文
              this.$set(this.where, 'cosText', res.data.cosText); //电子签章下弦文

              this.$set(this.where, 'returnAccountName', res.data.returnAccountName); //还款账户名
              this.$set(this.where, 'returnBankName', res.data.returnBankName); //还款开户支行
              this.$set(this.where, 'returnAccountNo', res.data.returnAccountNo); //还款账号

              this.$set(this.where, 'midAccountName', res.data.midAccountName); //居间费账户名
              this.$set(this.where, 'midBankName', res.data.midBankName); //居间费开户支行
              this.$set(this.where, 'midAccountNo', res.data.midAccountNo); //居间费账号
    ...
    saveAndSubmit() {
      ...
      if (!isValid) return false;
      //验证计划出款金额必须小于申请金额
      let applyMoney = this.where.applyMoney;
      let outputListVos = this.where.outputListVos;
      let outputMoney = 0;
      outputListVos.forEach(x => {
        outputMoney += parseFloat(x.outputMoney);
      });
      if (outputMoney > parseFloat(applyMoney)) {
        this.$message.error('出款金额必须等于或小于申请金额');
        return false;
      }
      //合并抵押物信息
      let houseMortgageSaveVo = this.$refs.InfoForContractAndOutMoney.houseMortgageList;
      this.$set(params, 'houseMortgageSaveVo', houseMortgageSaveVo);
      //提交
