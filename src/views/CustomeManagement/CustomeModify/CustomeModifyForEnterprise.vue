<template>
  <div class='apply-box-card'>
    <el-collapse v-model="activeNames">
      <el-collapse-item class="collapse-head" title="企业基本信息" name="1">
        <el-form :disabled="isView" :model='where' ref="baseInfo" size="small" :rules="rules" class='hongte-query-form' :inline='true' label-position="right" label-width="120px">
          <HtInput class="f-item3" label='客户名称' v-model="where.customerName" disabled></HtInput>
          ...
          <HtInput class="f-item3" label='统一社会信用代码' prop="unifiedSocialCreditCode" v-model="where.unifiedSocialCreditCode"></HtInput>
      ...
      <el-collapse-item title="企业法人" name="2">
        <el-form :disabled="isView" :model='where' ref="personInfo" size="small" :rules="rules" class='hongte-query-form' :inline='true' label-position="right" label-width="120px">
          ...
          <HtSelect class="f-item3" label='证件类型' type="certificate_type" v-model="where.enterpriseLegalCertificateTypeCode" :name.sync="where.enterpriseLegalCertificateTypeName" disabled></HtSelect>
          <HtInput class="f-item3" label='证件号码' v-model="where.enterpriseLegalCertificateNumber" disabled></HtInput>
          ...
          <HtInput class="f-item3" label='手机号码' v-model="where.phone" disabled></HtInput>
          ...
          <HtSelect class="f-item3" label='处罚情况' prop="punishSituation" v-model="where.punishSituation" :rules="rules.punishSituation" :name.sync="where.punishSituationName" :dataSource="punishSituationOptions"></HtSelect>
    ...
    <el-row class="bottom-btn">
      <el-button type="primary" v-if="!isView"  @click="submit">提交</el-button>
...
export default {
  name: 'CustomeModifyForEnterprise',
  data() {
    return {
  ...
  methods: {
      ...
      this.$api
        ...
        .then(res => {
          if (res.returnCode == '0000') {
            console.log(this.postData.customerCode);
    ...
    submitData() {
      //提交表单
      this.$api.updateCustomer(this.where).then(res => {
        if (res.returnCode == '0000') {
