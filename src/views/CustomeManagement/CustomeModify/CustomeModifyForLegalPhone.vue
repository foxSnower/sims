<template>
  <div>
    <el-form :model='where' size="small" ref="formName" :rules="rules" class='hongte-query-form' :inline='true' label-position="right" label-width="120px">
      ...
      <el-form-item label='法人姓名' class="f-item1">
        <el-input v-model='where.enterpriseLegalPerson' placeholder='请输入' disabled></el-input>
      </el-form-item>
      <el-form-item label='法人新手机号' prop="newValue" class="f-item1">
        <el-input v-model='where.newValue' placeholder='请输入'></el-input>
      </el-form-item>
      <el-form-item label='法人原手机号' class="f-item1">
...
import { CustomeModifyForLegalPhoneRules } from '@rules';
import { validator } from '@utils';
export default {
  name: 'CustomeModifyForLegalPhone',
  data() {
    return {
      where: {},
      rules: CustomeModifyForLegalPhoneRules
    };
  },
  props: ['postData'],
  watch: {
        ...
        if (valid) {
          this.$api
            .changeSensitiveInfo({
              ...
              type: '3' //1:变更客户名称, 2:变更客户手机号码, 3:更改法人手机号码 4:变更证件信息
            })
            .then(res => {
              if (res.returnCode == '0000') {
                this.$emit('closeDialog');
