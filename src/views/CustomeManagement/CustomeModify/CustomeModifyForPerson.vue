    ...
    <el-collapse v-model="activeNames">
      <el-collapse-item class="collapse-head" title="个人基本信息" name="1">
        <el-form :disabled="isView" :model='where' ref="personInfo" size="small" :rules="rules" class='hongte-query-form' :inline='true' label-position="right" label-width="120px">
          ...
          <HtSelect class="f-item3" label='证件类型' type="certificate_type" v-model="where.certificateTypeCode" :name.sync="where.certificateTypeName" disabled></HtSelect>
          <HtInput class="f-item3" label='证件号码' v-model="where.certificateNumber" disabled></HtInput>
      ...
      this.$api
        ...
        .then(res => {
          if (res.returnCode == '0000') {
            ...
            if (result) {
              if (result.customer) {
                this.$set(this.where, 'professionalNatureCode', '1');
              }
            }
          } else {
